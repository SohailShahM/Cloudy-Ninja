...
audio code lives here. Has no JVM-launcher or Android dependencies, so it
compiles identically against either runtime.

- Source root: [`core/src/main/kotlin/com/sohai/platformer/`](../core/src/main/kotlin/com/sohai/platformer/)
- Tests: [`core/src/test/kotlin/`](../core/src/test/kotlin/) (Kotest + MockK,
  ~600+ specs running across 4 parallel forks — see
  [`core/build.gradle`](../core/build.gradle))
- Build deps (see [`core/build.gradle`](../core/build.gradle)): libGDX
  (gdx, gdx-box2d, gdx-freetype), Box2DLights, kotlinx.serialization,
  VisUI, gdx-controllers. `ashley` and `gdx-ai` are declared but unused
  (tracked as T-127).

### `lwjgl3/`
Thin desktop launcher. Two files: a Kotlin `main`
([`Lwjgl3Launcher.kt`](../lwjgl3/src/main/kotlin/com/sohai/platformer/lwjgl3/Lwjgl3Launcher.kt))
that constructs `Main()` and runs it inside an `Lwjgl3Application`, plus
[`StartupHelper.java`](../lwjgl3/src/main/java/com/sohai/platformer/lwjgl3/StartupHelper.java)
which handles macOS `-XstartOnFirstThread` relaunch quirks.

The `:lwjgl3:dist` task is an alias for `:lwjgl3:jar` and produces the
release fat-JAR consumed by the itch.io deploy workflow.

### `android/`
Thin Android launcher.
[`AndroidLauncher.kt`](../android/src/main/kotlin/com/sohai/platformer/android/AndroidLauncher.kt)
extends `AndroidApplication`, enables immersive mode, and starts `Main()`.

Asset manifest [`assets/assets.txt`](../assets/assets.txt) is regenerated by
the root [`build.gradle`](../build.gradle) at build time — do not hand-edit.
...
### Constants live in `Constants.kt`
Tuning values — gravity, jump windows, collision bits, virtual size,
`BUILD_VERSION`, `BUILD_DATE` — all live in
[`Constants.kt`](../core/src/main/kotlin/com/sohai/platformer/Constants.kt).
Tune one at a time, playtest, write down the feel difference. Do not
scatter literals into other files.

### Hot-path allocations
`LevelRenderer` runs every frame. Do not allocate `Color` or `Vector2`
inside its draw methods — use the `Palette` companion or pre-allocated
temporaries. Same rule applies to `LevelRunState.update`.

### Tests bypass GL constructors
Box2D entities and screens that need real `SpriteBatch`/`Texture`
construction are constructed via reflection in tests
(`ObjenesisStd` / `sun.misc.Unsafe.allocateInstance`). MockK handles
libGDX statics (`Gdx.app`, `Gdx.audio`, `Gdx.files`) inside
`beforeSpec`/`afterSpec`. This pattern is established across
`StormSentinelTest`, `SmogSpriteTest`, `ProjectileTest`,
`DriftHuskTest`, `ParallaxBackgroundTest`, `ScreenFadeTest`, and the
manager specs.

### CI / smoke
[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs compile +
test + android lint on push/PR to `main`. Smoke uses the autopilot
(`-PcloudyAutopilot=true`) and `CrashReporter` is a no-op in smoke mode
so spurious exceptions don't write crash logs.
