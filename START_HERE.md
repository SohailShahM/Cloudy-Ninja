...
| `antigravity` | Tier S, M, L — exposes 6 model options each with their own quota bucket (Gemini 3 Flash + Pro low/high, Claude Sonnet/Opus 4.6, GPT-OSS 120B). **Flash is high-capacity and a real workhorse option** for bulk mechanical work; Pro/Opus are reserved for hard reasoning where Flash falls short. AGV picks the right model per turn and reports per-model green/yellow/red via `bridge_set_status`. Routing cheatsheet: [model-routing.md](https://github.com/SohailShahM/cc-agv-bridge/blob/main/docs/model-routing.md). Still coordinate with Claude Code via [cc-agv-bridge](https://github.com/SohailShahM/cc-agv-bridge). Report PR-comment status every 30 min if running >1 hour. |
...
Multi-model peer with 6 quota buckets (Gemini 3 Flash + Pro low/high, Sonnet/Opus 4.6 Thinking, GPT-OSS 120B). The routing principle: **default to Gemini 3 Flash** — it has the most capacity and is a real workhorse for bulk mechanical work. Escalate to Pro / Opus only when the reasoning load justifies burning the smaller bucket. Sonnet for non-Google second opinions. GPT-OSS as fallback. Full cheatsheet: [model-routing.md](https://github.com/SohailShahM/cc-agv-bridge/blob/main/docs/model-routing.md).

**Coordinating with Claude Code on the same work:** the [cc-agv-bridge](https://github.com/SohailShahM/cc-agv-bridge) MCP server is wired into both AGV (`~/.gemini/antigravity/mcp_config.json`) and Claude Code (`~/.claude.json`). At session start, paste the AGV starter prompt from [the bridge's docs/starter-prompts.md](https://github.com/SohailShahM/cc-agv-bridge/blob/main/docs/starter-prompts.md) — it calls `bridge_diagnose`, drains pending work, and **self-rates per-model capacity** via `bridge_set_status` (format: `capacity: flash=green, pro=yellow, sonnet=green, opus=red`). Verified live: Gemini has no quota-introspection tool, so coarse per-model buckets are the honest answer.

**Primary tools:** `bridge_ask`, `bridge_request_review`, `bridge_send`, `bridge_handoff`. CC may include a model hint in the message text (e.g. `"(prefer Pro for this — it's an architecture call)"`); AGV decides whether to honor based on its bucket state. **Decline handoffs that don't fit any green/yellow bucket** (`bridge_decline(handoff_id, reason="all Google buckets red, fallback would be Sonnet but task needs Pro-tier reasoning")`) rather than silently failing partway through.
...
| Code work where a second opinion / fresh-eyes review adds value | implementer = `claude-code-*` or `copilot-agent`; reviewer = `antigravity` (Flash) | Reviewer pulled in via [cc-agv-bridge](https://github.com/SohailShahM/cc-agv-bridge) `bridge_request_review` — Flash handles single-file reviews cheaply |
| Mid-flight ambiguity that needs the other agent's judgement | originator calls `bridge_ask` | Either direction; AGV-Flash makes asks effectively free |
| Bulk mechanical work when `copilot-agent` is busy / blocked | `antigravity` (Flash) via `bridge_handoff` | Flash is workhorse-tier — only escalate to Pro if cross-cutting |
